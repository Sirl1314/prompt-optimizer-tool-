...
**Prompt 智能优化对比工具** 是一款面向 AI 使用者的专业 Prompt Engineering 平台。它通过 **规则引擎 + LLM 混合优化管线**，自动检测原始 Prompt 中的结构缺陷、冗余表达和缺失要素，并针对不同垂直领域套用专属模板进行语义级精炼，最终输出结构清晰、表达精准的高质量 Prompt。

系统同时提供多维评分、Token 估算、多版本 Diff 对比和完整的历史管理能力，帮助用户持续迭代和优化 Prompt 质量。
